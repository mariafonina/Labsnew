...
// Create a new note
router.post('/', ...protectedTextSubmission({ maxDuplicates: 3, windowMs: 120000 }), asyncHandler(async (req: AuthRequest, res) => {
...
// Update a note
router.put('/:id', ...protectedTextSubmission({ maxDuplicates: 3, windowMs: 120000 }), asyncHandler(async (req: AuthRequest, res) => {
  const { id } = req.params;
  const { title, content, linked_item } = req.body;
