...
router.get('/', verifyToken, requireAdmin, asyncHandler(async (req: AuthRequest, res: Response) => {
  const result = await query(`
    ...
    LEFT JOIN labs.pricing_tiers pt ON c.id = pt.cohort_id AND pt.is_active = TRUE
...
router.post('/:productId/cohorts/:cohortId/tiers', verifyToken, requireAdmin, createLimiter, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const result = await query(`
    INSERT INTO labs.pricing_tiers (cohort_id, name, description, price, tier_level, features, access_start_date, access_end_date, is_active)
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
    RETURNING *
  `, [
    cohortId,
