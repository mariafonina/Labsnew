...
- **Content Management:** CRUD operations for instructions, events, favorites, notes, questions, and user progress. Direct image upload for admin news. **Object Storage Integration (Nov 2025):** RichTextEditor enhanced with three-tab image dialog (Upload/URL/Unsplash) via Replit Object Storage. Backend endpoints `/api/admin/objects/upload-url` and `/api/admin/objects/confirm-upload` handle signed URL generation and ACL confirmation. Images served via `/api/objects/*` endpoint with visibility control. Fallback mechanism uses REPL_ID-based bucket (`/${REPL_ID}/public`) when `PUBLIC_OBJECT_SEARCH_PATHS` and `PRIVATE_OBJECT_DIR` environment variables are not configured.
